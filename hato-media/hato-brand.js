(()=>{
  ...
  const boot=()=>{
    ...
    const PHOTO_POOL=['./assets/article-1.svg?v=2','./assets/article-2.svg?v=2','./assets/article-3.svg?v=2'];
...
.story-art,.article-hero-art{background:#111!important;overflow:hidden!important}.hato-news-photo{display:block!important;width:100%!important;height:100%!important;object-fit:cover!important}.article-hero-art .hato-news-photo{aspect-ratio:3/2!important}
#aboutPage>.about-birds img{width:min(330px,78vw)!important;height:auto!important;object-fit:contain!important}`;
    ...
    const makePhoto=a=>{const img=document.createElement('img');img.className='hato-news-photo';img.src=photoFor(a);img.alt='';img.loading='lazy';img.decoding='async';img.fetchPriority='low';img.onerror=()=>img.closest('.story-art,.article-hero-art')?.setAttribute('hidden','');return img;};
    ...
    const replaceNewsPhotos=()=>{document.querySelectorAll('#feed article[data-id]').forEach(card=>{const a=app.articles.find(x=>Number(x.id)===Number(card.dataset.id)),art=card.querySelector('.story-art');if(!a||!art)return;const wanted=new URL(photoFor(a),document.baseURI).href;const current=art.querySelector('.hato-news-photo');if(!current||current.src!==wanted){art.removeAttribute('hidden');art.replaceChildren(makePhoto(a));}});const hero=document.querySelector('#articleContent .article-hero-art'),title=document.querySelector('#articleContent .article-title')?.textContent?.trim();if(hero&&title){const a=app.articles.find(x=>x.title?.trim()===title);if(a){const wanted=new URL(photoFor(a),document.baseURI).href;const current=hero.querySelector('.hato-news-photo');if(!current||current.src!==wanted){hero.removeAttribute('hidden');hero.replaceChildren(makePhoto(a));}}}};
