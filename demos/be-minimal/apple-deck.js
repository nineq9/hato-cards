...
(() => {
  ...
  // Existing center trash marker is replaced by three directional targets.
  ...
  targets.innerHTML=`
    ...
    <div class="review-target review-target-keep" data-action="keep" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M12 20V5M6.5 10.5 12 5l5.5 5.5"/></svg></div>
  ...
  undo.className='apple-undo'; undo.setAttribute('aria-label','ひとつ戻す');
  ...
  function updateDeck(){
    ...
    undo.disabled=!history.length; updateReview();
  }
  function renderApple(){
    film.innerHTML=''; available().forEach(i=>{const p=photos[i],card=document.createElement('article');card.className='photo-card';card.dataset.i=i;card.innerHTML=`<img class="photo-media" src="${p.src}" alt=""><div class="size-badge">${signal()}${p.size}</div>`;film.appendChild(card);});
  ...
  function commit(card,action,dx,dy){
    ...
    let tx=0,ty=0,rot=0;if(action==='trash'){tx=-760;ty=Math.max(-90,Math.min(90,dy*.2));rot=-7;}else if(action==='later'){tx=760;ty=Math.max(-90,Math.min(90,dy*.2));rot=7;}else{tx=Math.max(-80,Math.min(80,dx*.15));ty=-900;rot=0;}
    ...
    card.animate([{transform:from,opacity:1},{transform:`translate(calc(-50% + ${tx}px),calc(-50% + ${ty}px)) rotate(${rot}deg) scale(.97)`,opacity:.03}],{duration:190,easing:'cubic-bezier(.20,.80,.20,1)',fill:'forwards'}).onfinish=()=>{
      ...
      history.push({i,action});clearHot();renderApple();
    };
  }
  ...
  undo.addEventListener('click',()=>{const last=history.pop();if(!last)return;if(last.action==='trash')queued.delete(last.i);else if(last.action==='keep')passed.delete(last.i);else later.delete(last.i);renderApple();});
  ...
  const originalCloseViewer=closeViewer;closeViewer=()=>{viewerIndex=null;resetViewerZoom(false);originalCloseViewer();};
  ...
  viewerImg.addEventListener('click',e=>{e.stopPropagation();if(viewerIndex===null)return;if(performance.now()<suppressViewerTapUntil||viewerGestureMoved){viewerGestureMoved=false;return;}const i=viewerIndex;queued.add(i);passed.delete(i);later.delete(i);history.push({i,action:'trash'});if(navigator.vibrate)navigator.vibrate(7);viewerIndex=null;resetViewerZoom(false);originalCloseViewer();renderApple();});
  renderApple();
})();