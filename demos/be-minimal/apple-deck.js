...
(() => {
  ...
  if(document.readyState==='complete')scheduleOpeningClose();
  else window.addEventListener('load',scheduleOpeningClose,{once:true});
  opening?.addEventListener('click',closeOpening);

  const targets=document.createElement('div');
  targets.className='review-targets';
  targets.innerHTML=`
    <div class="review-target review-target-trash" data-action="trash" aria-hidden="true">
      <svg viewBox="0 0 24 24"><path d="M5 7h14M9 7V5h6v2M8 9l.7 9h6.6L16 9M10 10v6M14 10v6"/></svg>
    </div>
    <div class="review-target review-target-keep" data-action="keep" aria-hidden="true">
      <svg viewBox="0 0 24 24"><path d="M7 4.5h10v15l-5-3.2-5 3.2z"/></svg>
    </div>
    <div class="review-target review-target-later" data-action="later" aria-hidden="true">
      <svg viewBox="0 0 24 24"><path d="M19 8a7 7 0 1 0 1 6"/><path d="M19 4v4h-4"/></svg>
    </div>`;
  ...
  function updateDeck(){
    ...
    counter.textContent=done>=photos.length?`${photos.length} / ${photos.length}`:`${done+1} / ${photos.length}`;
    targets.classList.toggle('is-complete',done>=photos.length);
    updateReview();
  }
  ...
  function resistance(v){
    const sign=v<0?-1:1,magnitude=Math.abs(v);
    return sign*(magnitude<190?magnitude*.97:184.3+(magnitude-190)*.44);
  }
  function actionFor(dx,dy,vx,vy){
    const ax=Math.abs(dx),ay=Math.abs(dy);
    if(dy<0&&(ay>ax*.68||vy<-650)&&(ay>50||vy<-720))return 'keep';
    if(dx<0&&(ax>56||vx<-620))return 'trash';
    if(dx>0&&(ax>56||vx>620))return 'later';
    return null;
  }
  function restore(card){
    ...
    ],{duration:260,easing:'cubic-bezier(.22,.78,.24,1)'}).onfinish=()=>{
      card.style.transform='translate(-50%,-50%)';card.style.opacity='';clearHot();
    };
  }
  function commit(card,action,dx,dy){
    const i=Number(card.dataset.i);
    setHot(action);
    if(navigator.vibrate)navigator.vibrate(action==='trash'?7:4);
    let tx=0,ty=0,rot=0;
    if(action==='trash'){tx=-760;ty=Math.max(-80,Math.min(80,dy*.18));rot=-6;}
    else if(action==='later'){tx=760;ty=Math.max(-80,Math.min(80,dy*.18));rot=6;}
    else{tx=Math.max(-70,Math.min(70,dx*.12));ty=-900;}
    const from=card.style.transform||'translate(-50%,-50%)';
    card.animate([
      {transform:from,opacity:1},
      {transform:`translate(calc(-50% + ${tx}px),calc(-50% + ${ty}px)) rotate(${rot}deg) scale(.97)`,opacity:.03}
    ],{duration:190,easing:'cubic-bezier(.20,.80,.20,1)',fill:'forwards'}).onfinish=()=>{
      if(action==='trash')queued.add(i);
      else if(action==='keep')passed.add(i);
      else later.add(i);
  ...
  film.addEventListener('pointerdown',e=>{
    const card=e.target.closest('.photo-card[data-depth="0"]');
    if(!card)return;
    if(e.pointerType==='mouse'&&e.button!==0)return;
    card.setPointerCapture?.(e.pointerId);
    active={card,id:e.pointerId,sx:e.clientX,sy:e.clientY,lx:e.clientX,ly:e.clientY,lt:performance.now(),vx:0,vy:0,moved:false,long:false,timer:null};
  ...
  film.addEventListener('pointermove',e=>{
    ...
    active.vx=(e.clientX-active.lx)/dt*1000;active.vy=(e.clientY-active.ly)/dt*1000;
    ...
    const x=resistance(dx),y=resistance(dy),rot=Math.max(-6,Math.min(6,x/29)),scale=1-Math.min(.03,Math.hypot(x,y)/3200);
    active.card.style.transform=`translate(calc(-50% + ${x}px),calc(-50% + ${y}px)) rotate(${rot}deg) scale(${scale})`;
    setHot(actionFor(dx,dy,active.vx,active.vy));
    e.preventDefault();e.stopImmediatePropagation();
  },true);

  function finish(e,cancel=false){
    if(!active||e.pointerId!==active.id)return;
    const a=active;active=null;clearTimeout(a.timer);e.stopImmediatePropagation();
    if(a.long){restore(a.card);return;}
    const dx=(e.clientX??a.lx)-a.sx,dy=(e.clientY??a.ly)-a.sy;
    if(!a.moved&&Math.abs(dx)<7&&Math.abs(dy)<7){a.card.style.transform='translate(-50%,-50%)';clearHot();openViewer(Number(a.card.dataset.i));return;}
    if(cancel){restore(a.card);return;}
    const action=actionFor(dx+a.vx*.075,dy+a.vy*.075,a.vx,a.vy);
    if(action){commit(a.card,action,dx,dy);return;}
    restore(a.card);
  }
  ...
  document.getElementById('viewerClose')?.addEventListener('click',()=>{viewerIndex=null;resetViewerZoom(false)});

  viewerImg.addEventListener('pointerdown',e=>{
    viewerImg.setPointerCapture?.(e.pointerId);viewerPointers.set(e.pointerId,{x:e.clientX,y:e.clientY});viewerGestureMoved=false;
    if(viewerPointers.size===2){pinchStartDistance=pointerDistance();pinchStartScale=viewerScale;viewerGestureMoved=true;}
    e.preventDefault();
  });
  viewerImg.addEventListener('pointermove',e=>{
    ...
    if(viewerPointers.size>=2&&pinchStartDistance>0){
      ...
      applyViewerScale(false);suppressViewerTapUntil=performance.now()+260;
    }
    e.preventDefault();
  });
  function endViewerPointer(e){
    if(!viewerPointers.has(e.pointerId))return;
    const wasPinching=viewerPointers.size>=2;viewerPointers.delete(e.pointerId);
    if(wasPinching){suppressViewerTapUntil=performance.now()+260;pinchStartDistance=0;if(viewerScale<1.03)resetViewerZoom(true);}
  }
  ...
  viewerImg.addEventListener('click',e=>{
    ...
    // Viewing never changes review state. Second tap only closes the preview.
    viewerIndex=null;resetViewerZoom(false);originalCloseViewer();
  });

  window.resetPhotoReview=()=>{queued.clear();passed.clear();later.clear();renderApple()};
  renderApple();
})();