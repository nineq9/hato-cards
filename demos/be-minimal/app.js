...
const basePhotos=[
  {src:"./demo-photo-yellow.webp",size:"1.2 MB",sizeMB:1.2,date:"2026年8月16日 09:41",place:""},
  {src:"./demo-photo-window.webp",size:"2.1 MB",sizeMB:2.1,date:"2026年8月16日 09:38",place:""},
  {src:"./demo-photo-swimmer.webp",size:"3.8 MB",sizeMB:3.8,date:"2026年8月16日 09:33",place:""}
];
...
function formatMB(value){if(value>=1024)return `${(value/1024).toFixed(2)} GB`;if(value>=10)return `${Math.round(value)} MB`;return `${value.toFixed(1)} MB`}
function updateReview(){
  ...
  if(sz)sz.textContent=formatMB(bytes);
  ...
  if(total)total.textContent=formatMB(bytes);
  if(summary)summary.textContent=`写真・動画 ${count}枚`;
  if(deleteBtn){deleteBtn.disabled=!photoReviewCompleted;deleteBtn.textContent=count?"削除を進める":"次へ";}
}
function updateCompletion(){
  document.getElementById("completionPhotos").textContent=`${photoDeletionConfirmed?deletedPhotoCount:0}枚`;
  document.getElementById("completionPhotoStorage").textContent=photoDeletionConfirmed?formatMB(deletedPhotoMB):"0 MB";
  document.getElementById("completionApps").textContent=`${appDeletionConfirmed?1:0}個`;
  document.getElementById("completionAppStorage").textContent=appDeletionConfirmed?"1.8 GB":"0 GB";
}
...
function showAppDeleteGuide(){
  appQueued=true;
  openDeleteSheet(`${appHead()}<button class="copy-app-name" id="copyAppName" type="button"><span>CapCutをコピー</span><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="8" y="8" width="10" height="10" rx="2"></rect><path d="M6 15H5a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h7a2 2 0 0 1 2 2v1"></path></svg></button><div class="delete-instructions"><div class="delete-instruction"><span class="delete-step">1</span><span>ホーム画面を下にスワイプして検索を開く</span></div><div class="delete-instruction"><span class="delete-step">2</span><span>「CapCut」を貼り付けて検索</span></div><div class="delete-instruction"><span class="delete-step">3</span><span>検索結果のCapCutを長押し</span></div><div class="delete-instruction"><span class="delete-step">4</span><span>「Appを削除」→ もう一度「Appを削除」</span></div></div><p class="delete-sheet-note">be minimalからApp本体を直接削除することはありません。</p><button class="delete-sheet-primary" id="appDeletedBtn">削除した</button><button class="delete-sheet-secondary" id="appLaterBtn">閉じる</button>`);
  document.getElementById("copyAppName").onclick=e=>copyAppName(e.currentTarget);
  document.getElementById("appDeletedBtn").onclick=()=>{
    appDeletionConfirmed=true;appQueued=false;updateCompletion();vibrate();
    ...
    setTimeout(()=>{closeDeleteSheet();go("completion")},220);
  };
  document.getElementById("appLaterBtn").onclick=closeDeleteSheet;
}
function confirmPhotosAndContinue(){
  deletedPhotoCount=queued.size;deletedPhotoMB=queuedMB();photoDeletionConfirmed=true;updateCompletion();go("afterPhotos");
}
...
function openGuideFromSwipe(){vibrate();appTrashHot(true);appCard.animate([{transform:appCard.style.transform||"none",opacity:1},{transform:"translate3d(0,34px,0) scale(.985)",opacity:.72}],{duration:130,easing:"cubic-bezier(.2,.8,.2,1)"}).onfinish=()=>{appCard.style.transform="";appCard.style.opacity="";appTrashHot(false);showAppDeleteGuide()}}
...
const screens={photos:document.getElementById("photosScreen"),final:document.getElementById("finalScreen"),afterPhotos:document.getElementById("afterPhotosScreen"),apps:document.getElementById("appsScreen"),completion:document.getElementById("completionScreen")};
function go(name){if(name==="final")updateReview();if(name==="completion")updateCompletion();Object.entries(screens).forEach(([k,s])=>s.classList.toggle("active",k===name));document.querySelectorAll(".nav-btn").forEach(b=>b.classList.toggle("active",b.dataset.target===name));mainNav.classList.toggle("is-hidden",name==="afterPhotos"||name==="completion")}
...
document.getElementById("appCleanupBtn").addEventListener("click",()=>go("apps"));
document.getElementById("finishWithoutAppsBtn").addEventListener("click",()=>{appDeletionConfirmed=false;updateCompletion();go("completion")});
